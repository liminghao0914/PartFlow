...
jQuery(document).ready(function () {
  ...
  jQuery.ajax({
    ...
    success: function(data) {
        ...
        console.log(data); 
        var button_list = jQuery('#button_list');
        var body = jQuery('body');
        for(var idx in data){
          var name = data[idx]['name']
          var content = data[idx]['content']
          button_list.append('<button id="'+name.replaceAll("\.","")+'">'+name+'</button>');
          body.append('<script type="bogus" id="'+name.replaceAll("\.","")+'_text">'+content+'</script>');
        }
        f();
    },
  ...
  function f(){
    var button_list = jQuery('#button_list');
    button_list.children("button").each(function (){
      console.log(this.id);
      $(this).on('click', function () {
        var id = this.id + '_text';
        InsertGraphvizText(jQuery("#"+id).html().trim());
      });
    });
